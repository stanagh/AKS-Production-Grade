...
namespace AttendanceCRM.Controllers
{
    ...
    public class StudentController : Controller
    {
        ...
        public StudentController(AttendanceCRMContext context, IHttpContextAccessor contextAccessor, IConfiguration configuration)
        {
            _context = context;
        ...
        [HttpPost]
        [ActionName("student")]
        public async Task<JsonResult> Student(string searchText = "")
        {
            DataTablesResponse<Users> result = new DataTablesResponse<Users>();
            List<Users> list = await _context.Users.Where(x => (x.IsStudent ?? false) == true).ToListAsync();
        ...
        [HttpGet]
        [ActionName("managestudent")]
        public async Task<ActionResult> ManageStudent(string data)
        {
            UserViewModel model = new UserViewModel();
            int decryptedId = 0;
            ...
            if (decryptedId > 0)
            {
                Users user = await _context.Users.FirstOrDefaultAsync(x => x.Id == decryptedId && (x.IsStudent ?? false) == true);
                model.User = user;
            ...
            return this.View("ManageStudent", model);
        }

        [HttpPost]
        [ActionName("managestudent")]
        public async Task<ActionResult> ManageStudent(UserViewModel userViewModel)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (model.Id > 0)
                    {
                        ...
                        this.TempData["SuccessMessage"] = string.Format(Messages.UpdateMessage, "Student details");
                        return this.Json(new { success = true, message = string.Format(Messages.UpdateMessage, "Student details") }, new Newtonsoft.Json.JsonSerializerSettings());
                    }
                    else
                    {
                        ...
                        this.TempData["SuccessMessage"] = string.Format(Messages.SaveSuccessMessage, "Student details");
                        return this.Json(new { success = true, message = string.Format(Messages.SaveSuccessMessage, "Student details") }, new Newtonsoft.Json.JsonSerializerSettings());

                    }
                }
                catch (Exception)
                {
                    return this.Json(new { success = false, message = string.Format(Messages.SaveErrorMessage, "Student details") }, new Newtonsoft.Json.JsonSerializerSettings());
                }
            }
        ...
        public async Task<ActionResult> DeleteStudent(string id)
        {
            ...
            {
                return this.Json(new { success = false, message = string.Format(Messages.DeleteErrorMessage, "Student") }, new Newtonsoft.Json.JsonSerializerSettings());
            }
            return this.Json(new { success = true, message = string.Format(Messages.DeleteMessage, "Student") }, new Newtonsoft.Json.JsonSerializerSettings());
        }
